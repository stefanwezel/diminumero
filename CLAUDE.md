...
- **Numeral systems** (`number_systems` in `languages/config.py`): a language may declare more than one way of saying its numbers. Welsh is the first — `decimal` (what schools teach, the deck that has always been there) and `traditional` (vigesimal; obligatory for the time, dates and age, per the r/learnwelsh review). Design notes:
  ...
  - **The completeness gate is derived from data, not a flag** (same principle as `_available_audio_numbers()`): a system declaring `requires_complete: (1, 100)` is offered only when every number in that range has a word. `None` entries in a deck mean "not verified yet" and are dropped at load. Welsh traditional ships 30 verified forms and 70 gaps, so the toggle is **not visible yet**; the PR that fills the last gap turns it on with no code change. This is what lets the control (phase 2) and the data (phase 4) land in either order.
  ...
  - The toggle is rendered **only** when a language has two *ready* systems (`templates/numbers.html`), as plain links so it works with no JS; single-system languages get no markup at all, not a disabled control. Labels come from `number_system_name_<key>` / `number_system_desc_<key>` (dynamic key lookup, like `magnitude_level_<n>`).
...
- **docs/QUESTIONS-FOR-NATIVE-SPEAKERS.md** — what we still don't know about traditional Welsh; the 41–99 gap blocks the toggle going live. *(Lands with the provenance follow-up.)*
