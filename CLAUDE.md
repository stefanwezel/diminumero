...
# Dump every form no speaker has confirmed individually (incl. the served
# `single` and `attested` tiers), as a markdown review table
...
- **models.py**: SQLAlchemy models. Five entities:
  - `Card(user_sub, front, back, times_practiced, times_correct, recent_results, created_at, updated_at, last_practiced_at)` — user-owned vocabulary card, free-form text on both sides (no per-card language). `recent_results` is a 10-char `'1'`/`'0'` string; the `score` property is `recent_results.count('1') / len(recent_results)` or `None` if unpracticed. `record_attempt(correct)` appends, trims, and stamps `last_practiced_at` — a separate column from `updated_at` precisely because `updated_at`'s `onupdate` fires on any row change, so editing a card's text would otherwise read as practising it.
  - `VerbCard(user_sub, lang, infinitive, times_practiced, times_correct, recent_results, created_at, updated_at)` — a verb a user added to their conjugation-practice pool for one language (`lang`, server-default `"es"`). Holds only the infinitive (conjugations come from that language's committed global pool, validated at add time). Same `score`/`record_attempt` scoring as `Card`, including `last_practiced_at`.
...
- **Numeral systems** (`number_systems` in `languages/config.py`): a language may declare more than one way of saying its numbers. Welsh is the first — `decimal` (what schools teach, the deck that has always been there) and `traditional` (vigesimal; obligatory for the time, dates and age, per the r/learnwelsh review). Design notes:
  ...
  - **The completeness gate is derived from data, not a flag** (same principle as `_available_audio_numbers()`): a system declaring `requires_complete: (1, 100)` is offered only when every number in that range has a word. `None` entries in a deck mean "not verified yet" and are dropped at load. Welsh traditional gates on `(1, 100)` — complete since the August 2026 follow-up round settled the 41-99 connective — so the toggle is live and the deck drills 1-100. A form cannot open a gate unless it is served, so `reconstructed` ones still can't; `attested` ones can, which is what raising the gate from `(1, 21)` to `(1, 100)` recorded. This is what lets the control (phase 2) and the data (phase 4) land in either order.
...
- **Form provenance** (`languages/provenance.py`, `config.SERVE_RECONSTRUCTED`): the Welsh traditional deck is being assembled from a public review thread, so every form records *who says so* — `confirmed` (two or more speakers), `single` (one, uncorroborated), `attested` (a rule a published reference states, cited by a reviewer — the *rule* has a source, not the individual form), or `reconstructed` (a rule with nothing published behind it; **nobody has checked it**). `SERVED_SOURCES` is the first three: the line is whether anything outside this repo backs the form, not whether a script typed it.
  - **Reconstructed forms are never served** while `SERVE_RECONSTRUCTED` is False (the default). A number whose only forms are reconstructed is *absent from the deck*: the drill skips it, does not fall back to the other system, and does not render a blank. Today that is 0 and 120 — everything else in 1-100 has a speaker or a source. They are committed for exactly two reasons — so they can be exported for review, and so they can be switched on in one line once confirmed.
  ...
  - `languages/cy/generate_numbers_traditional.py` encodes the rules (41-99 = `[unit] + connective + [score]`, 21-39 = `[unit] ar hugain`). `TENS_CONNECTIVE` decides 54 forms and is now answered — `a`, with the aspirate mutation, which only touches p/t/c and therefore cannot show on `deugain`; that is why `deg a thrigain` (70) and `tri a deugain` (43) looked like different rules and are not. The connective and the mutation it triggers are one choice (aspirate after `a`, soft after `ar`). `DOCUMENTED_CONNECTIVE` is separate from the switch on purpose: flipping the switch away from it demotes every 41-99 form from `attested` back to `reconstructed`, so the tier can never come to mean "whatever the generator last ran with". The generator **aborts** if the rule stops reproducing a confirmed form, and **reports** rather than resolves divergence from a speaker (Welsh 50, where the idiomatic `hanner cant` is taught over the regular `deg a deugain`). Its checks are re-asserted in `tests/test_cy_traditional.py` so a refactor that never reruns the script still fails.
...
- `/<lang_code>/listening` — Listening config screen (magnitude dial + share-link builder + Start), the listening twin of `/<lang_code>/numbers`. Also accepts shareable preset params (`range`, `magnitude` — there is no `mode` to pick here), which render the drill itself in that same response.
- `/<lang_code>/listen/start` — POST to initialize a Listening session from that screen's magnitude and range (audio-enabled languages only)
- `/<lang_code>/listen` — Listening mode quiz (GET/POST): plays the number's MP3, accepts a typed digit answer, supports reveal/next. With no listening session running it redirects to `/<lang_code>/listening`.
...
- **Weighted randomization**: Controlled by a user-facing magnitude dial (levels 1-5) on the mode selection page. Level 1 (default, decay=10) strongly favors small numbers; level 5 (decay=1) is uniform. The setting persists in the session so it carries across quizzes. Formula: `weight = (1/decay)^band` where bands are 0 (<100) through 4 (100K+). A deck may also declare `USAGE_WEIGHTS` (`{number: multiplier}`, read by `get_number_usage_weights()` and multiplied in) for a system whose numbers are not used evenly in the language itself: traditional Welsh holds 1-100, but dates keep 1-31 in daily use while `pedwar ar bymtheg ar hugain` (39) is a museum piece, and ten questions spent evenly would be ten questions spent on the wrong half. Every other deck declares nothing and draws exactly as before.
...
- **Card scoring**: Each `Card` keeps a 10-char `recent_results` history (most recent attempt last). `score` is the share of `'1'`s; `None` until first attempt. Cards with `0 ≤ score < 0.5` are "weak" and surface in the dashboard's weak-cards CTA. `_load_next_card()` enforces the no-repeat-within-session rule via `asked_ids` — which is per-round only, so spacing across rounds is the sampler's job, not the session's.
- **Prioritized sampling** (`_pick_weighted_card()` in `app.py`, shared by card *and* verb practice): weight = `((1 - score)² + 1/(1 + times_practiced)² + 0.05) × spacing`, where `spacing` ramps 1.0 → 4.0 over the two days since `last_practiced_at` (`_spacing_multiplier()`; a NULL timestamp means "no signal" and stays neutral at 1.0, so neither a brand-new card nor a row predating the column is mistaken for a stale one). **Both terms are squared on purpose.** The original linear form floored a mastered card at ~0.19 against a weak card's ~0.99 — barely 5:1 — and in a mature deck the mastered *majority* then took ~68% of every round on headcount alone, which is exactly what a user notices as "cards I already know keep coming up". Squaring widens the per-card gap to ~30:1. But no weight function alone fixes a deck that is 90% mastered — 55 small numbers still outweigh 5 large ones, and crushing mastered cards to zero would defeat retention. The spacing multiplier is what decides *which* mastered card returns: the one unseen for a fortnight rather than the one answered an hour ago. `tests/test_cards.py::TestPrioritizedSamplingAtDeckScale` pins the aggregate behaviour, because the per-card tests beside it use two-card decks and cannot see a deck-composition effect.
...
- **Listening overview (`/<lang>/listening`)**: the menu tile opens this config screen instead of dropping straight into a round, so a listening drill can be configured (magnitude, range) and shared exactly like a number drill. `_listening_deck()` returns `(system, playable_numbers)` — audio is per language, not per numeral system, so a system without MP3s falls back to the default deck — and **every bound on the page comes from the playable deck, not the number deck**: a range the MP3s don't cover would be a player with nothing to play. The dial is rendered only when the playable deck spans more than one magnitude, the same data-derived gate the number screen uses. `_start_listen_preset_drill()` is the listening twin of `_start_preset_drill()`; `mode` is deliberately **not** a preset param here (there is one mode), so `?mode=easy` renders the page rather than starting something. The Start form is pjax (`data-listen-pjax`) like every other entry into the quiz — the submitting click is what keeps the document "user activated", which is what lets the first clip autoplay — and `quiz_listen.js` anchors browser Back to this page. `/<lang>/numbers?mode=listening` still works: an already-shared link must not break.
...
Test files: `test_app.py` (quiz routes/session), `test_quiz_logic.py` (engine in isolation), `test_auth.py` (Auth0 login/callback/logout, mocked), `test_cards.py` (card CRUD, practice flow, scoring, sharing/import dedup, dashboard stats, and the prioritized sampler at deck scale — the mastered majority not dominating a round, strict weak > mid > mastered ordering, a stale card outranking a freshly-seen one, and an edit not counting as practice), `test_conjugate.py` (verb add/validate-against-pool/reject-unknown, autocomplete, practice flow + scoring, vosotros toggle, validate API, insights dashboard + `ConjugationStat` recording — against Spanish), `test_conjugate_de.py` (the German pool's forms incl. separable verbs/modals, `lang` scoping of the verbs API, es/de pool isolation, per-language sessions and dashboards), `test_conjugate_it.py` (the Italian pool's forms incl. -isc- verbs and avere/essere auxiliaries, it-pool scoping), `test_card_verb_sync.py` (index-card ↔ conjugation sync incl. language detection), `test_poll.py` (feedback poll endpoint and storage), `test_presets.py` (shareable drill links: valid params, out-of-range/garbage fallbacks, listening degraded on no-audio languages, cold no-session access, canonical/noindex), `test_listening.py` (the listening overview: the config screen and its playable-deck bounds, the mode-less share builder, shared `?range=`/`?magnitude=` links incl. a range with no MP3s in it, the Start form's range, availability redirects for languages without audio, and "back to overview" resolving to the listening screen after a listening round and the number screen otherwise), `test_worksheet.py` (printable worksheets: seed determinism incl. a pinned draw, seed minting/redirect, answer key matching the sheet, words drawn only from the verified deck, both directions, range/count fallbacks, print-only output, the required footer, the `?format=pdf` path — byte-identical output, answer key on its own PDF page, warning-free rendering, degradation when the PDF stack breaks — and the batch generator's seed stability), `test_number_systems.py` (languages with two numeral systems: single-system languages unaffected, the completeness gate, `?system=` on the config screen, on the language menu's tile picker and in shared links, session carry-over, the wrong-system nudge, partial ranges, sparse-deck multiple choice, worksheet cache-key separation, the usage weighting — dates outranking museum pieces, and the rarest tier still reachable — and the assertion that every served Welsh traditional form is either a speaker's or exactly what the documented rule produces), `test_notes.py` (per-number notes: every committed notes file validates, scope grammar, system scoping, translations and fallback, and the answer-leak rule from both ends — no note beside an unanswered prompt, and a mistaken `reveals_answer = false` fails the build), `test_cy_traditional.py` (form provenance: reconstructed forms never served and their numbers absent entirely, attested ones served, the flag flipping the rest on, gender propagation into compounds, the generator reproducing confirmed forms, the connective switch failing loudly against them *and* demoting its own output to reconstructed when it leaves the documented value, and divergence from a speaker reported not resolved), `test_translations.py` (locale key parity, orphan keys, placeholder counts, and that `PARTIAL_UI_LANGUAGES` agrees with actual coverage). Each test file still defines its own `app`/`client` fixtures.
